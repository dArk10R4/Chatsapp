  ...
  <div class="container border-1 border p-5 inline-block rounded mt-5">
    <form @submit.prevent="register">
      ...
      <div class="input">
        ...
        <input class="form-control" type="text" name="name" placeholder="Nihad" />
      </div>
      <div class="input">
        <label for="email">Email address</label>
        <input class="form-control" type="email" name="email" placeholder="email@adress.com" />
      </div>
      <div class="input">
      ...
      <div class="alternative-option mt-4">
        Already have an account? <span @click="moveToLogin" style="cursor: pointer; color: blue;">Login</span>
      </div>
      ...
      <div class="alert alert-warning alert-dismissible fade show mt-5 d-none" role="alert" id="alert_2">
        ...
        <button type="button" class="btn-close bg-info" data-bs-dismiss="alert" aria-label="Close"></button>
      </div>
    </form>
  </div>
</template>
...
export default {
  ...
  methods: {
    register(submitEvent) {
      ...
      axios.post(import.meta.env.VITE_APP_API_SERVER_URL + '/api/register',{
        "name": this.name,
        "email": this.email,
